...
config() {
    echo "sukad在修改配置后会自动尝试重启"
    vi /etc/sukad/config.json
    sleep 2
    restart
    check_status
    case $? in
        0)
            echo -e "sukad状态: ${green}已运行${plain}"
            ;;
        1)
            echo -e "检测到您未启动sukad或sukad自动重启失败，是否查看日志？[Y/n]" && echo
            read -e -rp "(默认: y):" yn
            [[ -z ${yn} ]] && yn="y"
            if [[ ${yn} == [Yy] ]]; then
               show_log
            ...
            ;;
        2)
            echo -e "sukad状态: ${red}未安装${plain}"
    esac
}

uninstall() {
    confirm "确定要卸载 sukad 吗?" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
    ...
    if [[ x"${release}" == x"alpine" ]]; then
        ...
        systemctl disable sukad
        rm /etc/systemd/system/sukad.service -f
        systemctl daemon-reload
        systemctl reset-failed
    fi
    rm /etc/sukad/ -rf
    rm /usr/local/sukad/ -rf

    echo ""
    echo -e "卸载成功，如果你想删除此脚本，则退出脚本后运行 ${green}rm /usr/bin/sukad -f${plain} 进行删除"
    echo ""
...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        echo -e "${green}sukad已运行，无需再次启动，如需重启请选择重启${plain}"
    else
        if [[ x"${release}" == x"alpine" ]]; then
            service sukad start
        else
        ...
        check_status
        if [[ $? == 0 ]]; then
            echo -e "${green}sukad 启动成功，请使用 sukad log 查看运行日志${plain}"
        else
            echo -e "${red}sukad可能启动失败，请稍后使用 sukad log 查看日志信息${plain}"
        fi
    fi
...
stop() {
    ...
    check_status
    if [[ $? == 1 ]]; then
        echo -e "${green}sukad 停止成功${plain}"
    else
        echo -e "${red}sukad停止失败，可能是因为停止时间超过了两秒，请稍后查看日志信息${plain}"
    fi
...
restart() {
    ...
    check_status
    if [[ $? == 0 ]]; then
        echo -e "${green}sukad 重启成功，请使用 sukad log 查看运行日志${plain}"
    else
        echo -e "${red}sukad可能启动失败，请稍后使用 sukad log 查看日志信息${plain}"
    fi
    if [[ $# == 0 ]]; then
...
status() {
    if [[ x"${release}" == x"alpine" ]]; then
        service sukad status
    else
        systemctl status sukad --no-pager -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        echo -e "${green}sukad 设置开机自启成功${plain}"
    else
        echo -e "${red}sukad 设置开机自启失败${plain}"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        echo -e "${green}sukad 取消开机自启成功${plain}"
    else
        echo -e "${red}sukad 取消开机自启失败${plain}"
    fi
...
show_log() {
    if [[ x"${release}" == x"alpine" ]]; then
        ...
        journalctl -u sukad.service -e --no-pager -f
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
check_enabled() {
    if [[ x"${release}" == x"alpine" ]]; then
        temp=$(rc-update show | grep sukad)
        if [[ x"${temp}" == x"" ]]; then
            return 1
        else
            return 0
        ...
        temp=$(systemctl is-enabled sukad)
...
check_uninstall() {
    ...
    if [[ $? != 2 ]]; then
        echo ""
        echo -e "${red}sukad已安装，请不要重复安装${plain}"
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        return 1
...
show_status() {
    ...
    case $? in
        0)
            ...
            ;;
        1)
            echo -e "sukad状态: ${yellow}未运行${plain}"
            show_enable_status
            ;;
        2)
            echo -e "sukad状态: ${red}未安装${plain}"
...
show_usage() {
    ...
    echo "sukad x25519       - 生成 x25519 密钥"
    echo "sukad generate     - 生成 sukad 配置文件"
    echo "sukad update       - 更新 sukad"
    echo "sukad update x.x.x - 安装 sukad 指定版本"
...
————————————————
  ...
  ${green}2.${plain} 更新 sukad
  ${green}3.${plain} 卸载 sukad
————————————————
  ${green}4.${plain} 启动 sukad
  ${green}5.${plain} 停止 sukad
  ${green}6.${plain} 重启 sukad
  ${green}7.${plain} 查看 sukad 状态
  ${green}8.${plain} 查看 sukad 日志
————————————————
  ${green}9.${plain} 设置 sukad 开机自启
  ${green}10.${plain} 取消 sukad 开机自启
————————————————
  ${green}11.${plain} 查看 sukad 版本
  ${green}12.${plain} 升级 sukad 维护脚本
  ${green}13.${plain} 生成 sukad 配置文件
 ...
 #后续更新可加入上方字符串中
    ...
    case "${num}" in
        ...
        11) check_install && show_sukad_version ;;
        12) update_shell ;;
...
if [[ $# > 0 ]]; then
    case $1 in
        ...
        "version") check_install 0 && show_sukad_version 0 ;;
